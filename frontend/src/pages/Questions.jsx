...
const Questions = () => {
  ...
  const difficulties = [
    { level: 'Easy', time: '20 mins', color: 'text-emerald-500', border: 'border-emerald-500', bg: 'bg-emerald-500/10', desc: 'Basic logic and conceptual checks.' },
    { level: 'Medium', time: '30 mins', color: 'text-amber-500', border: 'border-amber-500', bg: 'bg-amber-500/10', desc: 'Intermediate problem-solving puzzles.' },
    { level: 'Hard', time: '45 mins', color: 'text-accent', border: 'border-accent', bg: 'bg-accent/10', desc: 'Hardcore challenges and trick questions.' },
  ];

  const startPractice = (level) => {
    navigate(`/practice?category=questions&difficulty=${level.level}`);
  };
    ...
    <div className="space-y-12 animate-fade-in pb-24 relative">
      ...
      <header className="space-y-4">
        <h1 className="text-5xl font-bold tracking-tight text-text-main">
          Challenge <span className="text-primary">Bank</span>
        </h1>
        <p className="text-text-muted text-lg max-w-2xl font-medium">
          Access a curated repository of technical challenges designed to refine your specific problem-solving skills.
        </p>
      </header>
