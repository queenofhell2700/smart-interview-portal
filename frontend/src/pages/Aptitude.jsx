...
const Aptitude = () => {
  ...
  const difficulties = [
    { level: 'Easy', time: '20 mins', color: 'text-emerald-500', border: 'border-emerald-500', bg: 'bg-emerald-500/10', desc: 'Basic math and logical puzzles.' },
    { level: 'Medium', time: '30 mins', color: 'text-amber-500', border: 'border-amber-500', bg: 'bg-amber-500/10', desc: 'Quantitative questions and data interpretation.' },
    { level: 'Hard', time: '45 mins', color: 'text-accent', border: 'border-accent', bg: 'bg-accent/10', desc: 'Advanced reasoning and complex problems.' },
  ];

  const startPractice = (level) => {
    navigate(`/practice?category=aptitude&difficulty=${level.level}`);
  };

  return (
    <div className="space-y-12 animate-fade-in pb-24 relative">
       {/* Background Gradients */}
       <div className="absolute top-0 left-1/4 w-[500px] h-[500px] bg-primary/5 blur-[120px] rounded-full -z-10 animate-pulse"></div>

      <header className="space-y-4">
        <h1 className="text-5xl font-bold tracking-tight text-text-main">
          Cognitive <span className="text-primary">Aptitude</span>
        </h1>
        <p className="text-text-muted text-lg max-w-2xl font-medium">
          Practice logical reasoning and quantitative problems to sharpen your problem-solving skills.
        </p>
      </header>
